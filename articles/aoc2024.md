...
pub fn build(b: *std.Build) void {
    ...
    for (1..26) |day| {
        const day_zig_file = b.path(b.fmt("src/days/day));
        ...
        run_exe.root_module.addAnonymousImport("day", .);
...
Here’s the idea: map `'a'` to 0, `'b'` to 1, up to `'z'` as 25. Each character in the string becomes a digit in the base-26 number. For example,  `"bc"` ( `[2]u8`) becomes the base-10 number 28 ($1 × 26 + 2 = 28$). If we represent this using the base-64 character set, it becomes 12 (`'b'` = 1, `'c'` = 2).

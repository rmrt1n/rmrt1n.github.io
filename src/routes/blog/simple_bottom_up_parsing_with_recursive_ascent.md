...
In my research, I stumbled upon the term _recursive ascent parser_. This parsing
...
means to advance or _shift_ to the next token in the token stream. Reduce on the
other hand means to convert or _reduce_ shifted tokens into the grammar rules
...
| 8    | expr       | + 1       | shift                       |
| 9    | expr +     | 1         | shift                       |
| 10   | expr + 1   | _eof_     | reduce (num -> 1)           |
| 11   | expr + num | _eof_     | reduce (expr -> expr + num) |
| 12   | expr       | _eof_     | accept                      |
...
items in the **_canonical collection of lr(0) items_**. So to build this automaton,
...
nonterminal is called _applying closure_. So from the augmented grammar from
...
next symbol after the dot. The only symbols that can come next are the _expr_
nonterminal, _num_ nonterminal, and the '1' terminal symbol.

**state 1**  
The state if an _expr_ is given to state 0.
...
**state 2**  
The state if a _num_ is given to state 0.
...
**state 5**  
The state if a _num_ is given to state 4.
...
could result in a _shift-reduce_ conflict. This conflict happens when a table
...
S expressions, or usually called _sexprs_, are a way to represent nested data
